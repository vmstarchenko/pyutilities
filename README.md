...
## Небольшие находки
  ...
  - Про метаклассы: [habr](https://habrahabr.ru/post/145835/).  Очень понятно и
    доходичво. Там же рассказано про "волшебную" функцию `type`.

## Scripts:
  - `update_cache.py`: скачивает страницы и сохраняет в папке `.page_cache`. В
    качестве конфига использует файл `cache_config.json`, закисаный как пары
