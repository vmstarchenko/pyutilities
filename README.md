...
## Небольшие находки
  ...
  - Бывает, возникает такая ошибка при объявлении кортежа:
    ```python
    x = ('hello')  # Ошибка. То же самое что и x = 'hello'
    x = ('hello',) # Правильно. То же что и x = tuple(['hello'])
    ```

    Поэтому даже если объявляется кортеж более чем из одного элемента, лучше
    всегда после последнего элемента ставить запятую.

  - Про [метаклассы](https://habrahabr.ru/post/145835/). Очень понятно и
    доходичво. Там же рассказано про "волшебную" функцию `type`. Однако, начиная
    с версии 3.0 синтаксис определения метакласса изменился. Вместо:
